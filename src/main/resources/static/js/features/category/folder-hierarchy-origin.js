...
function createFolder(categoryId, folderData) {
    ...
    const csrfInfo = getCsrfInfo(); // getCsrfInfo 함수 직접 호출
    ...
    // fetch 요청 로그 (디버깅용)
    console.log('요청 URL:', `/blog/${blogId}/category/${categoryId}/folder/create`);
    console.log('요청 헤더:', headers);
    console.log('요청 본문:', JSON.stringify(folderData));
    ...
    if (folder.parentFolderId) {
        ...
        if (!childContainer) {
            ...
            if (parentFolderItem) {
                ...
                if (folderIcon) {
                    folderIcon.className = 'bi bi-folder2 me-1';
                }
            } else {
...
// 모듈 내보내기
export { createFolder, addFolderToUI };