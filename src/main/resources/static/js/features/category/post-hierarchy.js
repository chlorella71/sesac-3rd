/**
 * 포스트 계층 구조를 위한 프론트엔드 JavaScript
 * 파일 경로: /static/js/features/category/post-hierarchy.js
 */
 ...
 * @param {string} folderId - 폴더 ID
 */
export function renderFolderPostsList(container, posts, folderId) {
    // 최대 5개만 표시
        ...
        .then(data => {
            if (data.success && data.posts) {
                if (data.posts.length > 0) {
                    // 포스트 목록 렌더링
                    renderPostsInMainContent(data.posts, postList, postInfoMessage);
                } else {
                    // 포스트가 없는 경우
 ...
 */
export function renderPostsInMainContent(posts, container, infoElement) {
    // 컨테이너 초기화
    container.innerHTML = '';
